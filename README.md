<p align="center">
  <img src="src/ZedWorkspaces/Images/plain-icon.png" alt="Zed Workspaces logo" width="128" />
</p>

<h1 align="center">Zed Workspaces</h1>
...
<p align="center">
  <br>
  <img src="assets/screenshot.png" alt="Zed Workspaces demo" width="600" />
  <br>
</p>
...
1. Download `ZedWorkspaces-<arch>.zip` from [Releases](https://github.com/JianJroh/PowerToysRun-ZedWorkspaces/releases) — pick `win-x64` or `win-arm64` to match your CPU.
2. Extract it and copy the resulting `ZedWorkspaces\` folder (containing `ZedWorkspaces.dll` and `plugin.json`) into:

   ```
   %LOCALAPPDATA%\Microsoft\PowerToys\PowerToys Run\Plugins\
   ```

   Final layout: `...\Plugins\ZedWorkspaces\plugin.json`
...
In *PowerToys Settings → PowerToys Run → Plugins → Zed Workspaces*:
...
```powershell
dotnet build src\ZedWorkspaces\ZedWorkspaces.csproj -c Release -p:Platform=x64 -r win-x64      # x64
dotnet build src\ZedWorkspaces\ZedWorkspaces.csproj -c Release -p:Platform=ARM64 -r win-arm64  # ARM64
```

Output: `src\ZedWorkspaces\bin\<Platform>\Release\net9.0-windows10.0.26100.0\win-<arch>\`
...
| Plugin doesn't load | Confirm the folder contains `ZedWorkspaces.dll` and `plugin.json`, then fully restart PowerToys. |
...
> The plugin reads recent projects from Zed's workspace history in `%LOCALAPPDATA%\Zed\db\<channel>\db.sqlite`, opened read-only. The schema has changed across Zed versions; if a future Zed release breaks the plugin, open an [issue](https://github.com/JianJroh/PowerToysRun-ZedWorkspaces/issues).
